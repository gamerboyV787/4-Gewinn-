...
const Stickman = (() => {
  ...
  function bindTouchControls() {
    ...
    wrap.querySelectorAll('button[data-touch]').forEach(btn => {
      ...
      btn.addEventListener('mousedown',   (e) => { e.preventDefault(); start(btn); });
      btn.addEventListener('mouseup',     (e) => { e.preventDefault(); stop(btn); });
      btn.addEventListener('mouseleave',  (e) => { e.preventDefault(); stop(btn); });
      ...
      btn.addEventListener('touchcancel', (e) => { e.preventDefault(); stop(btn); }, { passive:false });
    });
  ...
  function update() {
    players.forEach((p, idx) => {
      ...
      let fire = anyDown(p.controls.fire) || touchState[idx].fire;
      let leftIn = left, rightIn = right, jumpIn = jump;

      if (_bot && idx === 1) {
        const enemy = players[0];
        leftIn = p.x > enemy.x + 30;
        rightIn = p.x < enemy.x - 30;
        jumpIn = Math.abs(enemy.x - p.x) < 120 && Math.random() < (_bot==='easy'?0.005:_bot==='medium'?0.01:0.018);
        fire = Math.random() < (_bot==='easy'?0.03:_bot==='medium'?0.05:_bot==='hard'?0.07:0.09);
      }

      const inputX = (leftIn ? -1 : 0) + (rightIn ? 1 : 0);
      p.vx += inputX * 0.58;
      p.vx *= 0.79;
      ...
      if (jumpIn && grounded && !p.jumpLock) {
        p.vy = -11.8;
        p.jumpLock = true;
      }
      if (!jumpIn) p.jumpLock = false;
